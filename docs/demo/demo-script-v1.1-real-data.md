# Demo script — "Is This Real?" (v1.1)

Version 1.1. **Supersedes `demo-script-v1.0.md`**, which is left untouched per standing
rule 10. Track 2B, DEIK.AI Challenge 2026. Runtime budget: **7 minutes**.

## What changed from v1.0, and why

v1.0 was written before the real Green Sentinel export was loaded. Four of its figures
and one of its **verdicts** described the synthetic corpus, not the network we are
pitching about. Every change below is traceable to
`docs/updates/u22-headline-reconciliation.md`, which carries the commands and their
verbatim output.

| Passage | v1.0 said | v1.1 says | Why |
|---|---|---|---|
| Cold open | ~~99.95% complete~~ | **100.00% conventional completeness** | 99.95% was the *synthetic* corpus's grid completeness (99.9518%). The real drop measures 100.0000% conventional / 85.7374% grid. Neither is 99.95%. |
| B1 headline | (no rate stated for the real export) | **29.1225%**, of 174,583 covered cells | The measured real-drop rate. |
| B1 | — | the **R01 split**, volunteered | 48.97% of the headline is data that never arrived. Better said by us than found by a judge. |
| B3 verdict | ~~AMBIGUOUS, routed to human review~~ | **LIKELY_FAULT, confidence high, not routed to review** | **v1.0 was simply wrong about the real drop.** The adjudicator returns `LIKELY_FAULT` on KER11 with `match_score` 0.0 against a 0.2 fault threshold. The AMBIGUOUS claim described the *synthetic* corpus's top event. |
| B3 trust | ~~0.577 → 0.275~~ | **0.7347 → 0.4308, T04 appears** | Synthetic figures. The real stored trust series for KER11 drops 0.7347 (2026-06-02) → 0.4308 (2026-06-03) and gains reason code T04. |

Two figures in v1.0's preamble are **carried over unverified** and flagged rather than
restated as fact: the offline corpus's "75,585 readings, 2.906% defect rate" disagrees
with `u6-real-drop.md`'s measured synthetic figures (35,265 readings, 2.9620%). They are
different fixture invocations; nobody has reconciled them. Run `prov demo rehearse`
before relying on either. This does not touch the on-stage story, which is real-export
only.
...
*Title card: the mark, "AI Trust Layer for Environmental Data", the hook "Is This Real?".*
