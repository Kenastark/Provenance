# 3-minute video storyboard (v1.1)

Version 1.1. **Supersedes `video-storyboard-v1.0.md`** (untouched, per standing rule 10).
Target runtime **3:00**. Fully offline to record (`make demo-record`).

**Changed from v1.0:** shot 2's completeness figure (~~99.95%~~ → **100.00% conventional**
— 99.95% was the synthetic corpus), shot 5's trust figures (~~0.577 → 0.275~~ →
**0.73 → 0.43**, the real stored series), and shot 6's **verdict**, which v1.0 had wrong:
the real drop returns **LIKELY_FAULT**, not AMBIGUOUS, and it does *not* route to review.
Derivation: `docs/updates/u22-headline-reconciliation.md`.
