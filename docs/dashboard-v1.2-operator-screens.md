# Dashboard v1.2 — the operator screens

Phase 3, after the second flag review. **Supersedes
`dashboard-v1.1-operator-screens.md`.**

What changed since v1.1:

- **The demo map shows real streets.** A Debrecen OpenStreetMap basemap is fetched
  at setup and rendered under the markers; the default remains the token ground. See
  ADR 0006, and "Running it" below.
- **Defect list truncation is surfaced, never silent.** The cursor walk reports when
  it stops at its page cap, and the evidence panel says so rather than presenting a
  prefix as the whole set.
- **Trust reason codes carry their figures** (already true in v1.1) and the
  invariant that no two components share an evidence key is now pinned by a test.
...
## Running it
    ...
    make basemap       # fetch the Debrecen street basemap (once; offline after)
...
- **The basemap** now fetches a real Debrecen street basemap at setup (`make
  basemap`), rendered under the markers when present; the default is still the token
  ground. Offline after the first fetch, never committed. See ADR 0006. Street
  *labels* are omitted (the offline build carries no glyph fonts) — a documented
